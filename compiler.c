...
#define DIVIDE 12
#define PRINT 13
#define LEFTBRACKET 14
#define RIGHTBRACKET 15
#define STRING 16
#define DOUBLEQ 17
#define READINT 18
...
void getNextChar(void)
{
    ...
    if(currentChar == '/' && inputLine[currentColumnNumber] == '/' && bool == FALSE)
    {
            currentChar = '\n';
    }
}
...
TOKEN *getNextToken(void)
{
    ...
    //check for and create string image.
...
void printArg(void)
{
    ...
    if(currentToken -> kind == STRING)
    {
        t = currentToken;
        consume(STRING);
        strcpy(c, "^"); // creates special  char
        strcpy(label, getLabel()); //creates label puts it in label
        emitInstruction2("pc", label); //push on stack.
        emitInstruction1("sout");
        strcpy(temp, label); // saves label in temp
        strcpy(label, c); // puts caret in front of label
        strcat(label, temp); //put label in front of caret
        emitdw(label, t -> image);
    }
    else{
        expr();
